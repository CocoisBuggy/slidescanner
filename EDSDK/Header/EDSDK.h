...
//      Out:    outDataType - Pointer to the buffer that is to receive the
property
//                        type data.
...
//  Returns:    Any of the sdk errors.
-----------------------------------------------------------------------------*/
EdsError EDSAPI EdsGetPropertyDescEx(EdsBaseRef inRef,
                                     EdsPropertyID inPropertyID,
                                     EdsPropertyDescEx* outPropertyDesc);
...
/*-----------------------------------------------------------------------------
//
//  Function:   EdsCreateFolder
//
//  Description:
//       Create a folder in the camera's media.
//
//  Parameters:
//       In:    inCameraRef - The reference of the camera which will receive the
//                      command.
...
//  Returns:    Any of the sdk errors.
-----------------------------------------------------------------------------*/
EdsError EDSAPI EdsCreateFolder(EdsCameraRef inCameraRef);
...
//       In:    inCameraRef - The reference of the camera which will receive the
//                      command.
//      Out:    outFlashRef - The reference of the Flash.
//
//  Returns:    Any of the sdk errors.
-----------------------------------------------------------------------------*/
EdsError EDSAPI EdsCreateFlashSettingRef(EdsCameraRef inCameraRef,
...
//  Returns:    Any of the sdk errors.
-----------------------------------------------------------------------------*/
EdsError EDSAPI EdsGetDirectoryItemInfo(EdsDirectoryItemRef inDirItemRef,
...
//      The downloaded file is sent directly to a file stream created in
advance.
...
//
//  Description:
//      Extracts and downloads thumbnail information from image files in a
camera.
...
//
//  Description:
//      Writes information to the metadata of an image (Jpeg only) in the
camera.
//
//  Parameters:
//       In:    inDirItemRef - The reference of the directory item. (Jpeg only)
//              inMetaType - Designate the type of metadata to be set (ExifGPS
or XMP)
...
//                                and which action to take when files do not
exist.
//              inDesiredAccess - Access to the stream (reading, writing, or
both).
//      Out:    outStream - The reference of the stream.
//
//  Returns:    Any of the sdk errors.
-----------------------------------------------------------------------------*/
EdsError EDSAPI
EdsCreateFileStream(const EdsChar* inFileName,
...
//  Returns:    Any of the sdk errors.
-----------------------------------------------------------------------------*/
EdsError EDSAPI EdsCreateMemoryStream(EdsUInt64 inBufferSize,
                                      EdsStreamRef* outStream);
...
//      Unlike the buffer size of streams created by means of
EdsCreateMemoryStream,
//      the buffer size you prepare for streams created this way does not
expand.
//
//  Parameters:
...
//  Returns:    Any of the sdk errors.
-----------------------------------------------------------------------------*/
EdsError EDSAPI EdsCreateMemoryStreamFromPointer(EdsVoid* inUserBuffer,
...
stream.
//      As the EDSDK automatically resizes the buffer, the memory stream
provides
...
//
//  Parameters:
//       In:    inStream - Designate the memory stream for the pointer to
retrieve.
...
//      Out:    outBuffer - Pointer to the user-supplied buffer that is to
receive
...
//  Returns:    Any of the sdk errors.
-----------------------------------------------------------------------------*/
EdsError EDSAPI EdsRead(EdsStreamRef inStreamRef,
                        EdsUInt64 inReadSize,
                        EdsVoid* outBuffer,
...
//  Returns:    Any of the sdk errors.
-----------------------------------------------------------------------------*/
EdsError EDSAPI EdsWrite(EdsStreamRef inStreamRef,
...
//              inSeekOrigin - Pointer movement mode. Must be one of the
following
//                             values.
...
//  Returns:    Any of the sdk errors.
-----------------------------------------------------------------------------*/
EdsError EDSAPI EdsSeek(EdsStreamRef inStreamRef,
...
//  Returns:    Any of the sdk errors.
-----------------------------------------------------------------------------*/
EdsError EDSAPI EdsGetPosition(EdsStreamRef inStreamRef,
...
//  Returns:    Any of the sdk errors.
-----------------------------------------------------------------------------*/
EdsError EDSAPI EdsCopyData(EdsStreamRef inStreamRef,
                            EdsUInt64 inWriteSize,
...
//      This timing can be used in updating on-screen progress bars, for
example.
//
//  Parameters:
...
//  Returns:    Any of the sdk errors.
-----------------------------------------------------------------------------*/
EdsError EDSAPI EdsSetProgressCallback(EdsBaseRef inRef,
...
//
//  Parameters:
//       In:    inStreamRef - Designate the object for which to get image
information.
//              inImageSource - Of the various image data items in the image
file,
...
//  Returns:    Any of the sdk errors.
-----------------------------------------------------------------------------*/
EdsError EDSAPI EdsGetImage(EdsImageRef inImageRef,
                            EdsImageSource inImageSource,
...
//
//  Parameters:
//      In:     inStreamRef - The stream reference which opened to get EVF JPEG
image.
//      Out:    outEvfImageRef - The EVFData reference.
//
//  Returns:    Any of the sdk errors.
-----------------------------------------------------------------------------*/
EdsError EDSAPI EdsCreateEvfImageRef(EdsStreamRef inStreamRef,
...
//  Description:
//		Downloads the live view image data set for a camera currently in
live view mode.
//		Live view can be started by using the property
ID:kEdsPropertyID_Evf_OutputDevice and
//		data:EdsOutputDevice_PC to call EdsSetPropertyData.
//		In addition to image data, information such as zoom, focus
position, and histogram data
//		is included in the image data set. Image data is saved in a
stream maintained by EdsEvfImageRef.
//		EdsGetPropertyData can be used to get information such as the
zoom, focus position, etc.
//		Although the information of the zoom and focus position can be
obtained from EdsEvfImageRef,
...
//                          the callback function pointed to by CallBack
parameter.
//      Out:    None
//
//  Returns:    Any of the sdk errors.
-----------------------------------------------------------------------------*/
EdsError EDSAPI
EdsSetCameraAddedHandler(EdsCameraAddedHandler inCameraAddedHandler,
                         EdsVoid* inContext);
...
-----------------------------------------------------------------------------*/
EdsError EDSAPI
EdsSetPropertyEventHandler(EdsCameraRef inCameraRef,
...
//              inObjectEventHandler - Designate the pointer to the callback
function
...
-----------------------------------------------------------------------------*/
EdsError EDSAPI
EdsSetObjectEventHandler(EdsCameraRef inCameraRef,
...
//              inStateEventHandler - Designate the pointer to the callback
function
...
-----------------------------------------------------------------------------*/
EdsError EDSAPI
EdsSetCameraStateEventHandler(EdsCameraRef inCameraRef,
                              EdsStateEvent inEvnet,
                              EdsStateEventHandler inStateEventHandler,
                              EdsVoid* inContext);

/*----------------------------------------------------------------------------*/
EdsError EDSAPI EdsCreateStream(EdsIStream* inStream,
                                EdsStreamRef* outStreamRef);
...
//
//  Description:
//      Specifies the camera's focus and zoom frame position in the LiveView
state.
//
//  Parameters:
//       In:    inCameraRef - Designate the camera object.
//              inFramePoint - Specifies the position coordinates of the zoom
and AF
//                  frames of a LiveView image.
//              inLockAfFrame - Whether to lock the movement of the frame until
the
...
//  Returns:    Any of the sdk errors.
-----------------------------------------------------------------------------*/
EdsError EDSAPI EdsSetFramePoint(EdsCameraRef inCameraRef,
                                 EdsPoint inFramepoint,
                                 bool inLockAfFrame);
